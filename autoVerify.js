...
const { updateELOForPick } = require('./elo')
  ...
  for (const pick of picks) {
    try {
      ...
      if (result) {
        ...
        // Update ELO after grading
        await updateELOForPick(pick.id)
...
async function verifyTennisPicks() {
  const picks = await prisma.pick.findMany({ where: { result: 'pending', sport: 'Tennis' } })
  if (!picks.length) return
  for (const pick of picks) {
    try {
      const leagues = ['tennis_atp_french_open', 'tennis_wta_french_open']
...
cron.schedule('*/10 * * * *', async () => {
  console.log('\n🤖 Auto-verify + ELO running at', new Date().toLocaleTimeString())
  try {
    await verifySoccerPicks()
    await verifyNBAPicks()
    await verifyTennisPicks()
    console.log('✅ Auto-verify + ELO complete\n')
  } catch (err) {
    console.log('❌ Error:', err.message)
  }
})

console.log('🤖 Auto-verify + ELO system started')
